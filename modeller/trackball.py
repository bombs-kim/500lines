...
class Trackball:
    ...
    matrix = property(
        _get_matrix, doc="""Model view matrix transformation (read-only)"""
    )
    ...
    zoom = property(_get_zoom, _set_zoom, doc="""Zoom factor""")
    ...
    distance = property(
        _get_distance, _set_distance, doc="""Scene distance from point of view"""
    )
    ...
    theta = property(
        _get_theta, _set_theta, doc="""Angle (in degrees) around the z axis"""
    )
    ...
    phi = property(_get_phi, _set_phi, doc="""Angle around x axis""")
    ...
    def _get_orientation(self):
        ...
        ax = (
            math.atan(2 * (q0 * q1 + q2 * q3) / (1 - 2 * (q1 * q1 + q2 * q2)))
            * 180.0
            / math.pi
        )
        az = (
            math.atan(2 * (q0 * q3 + q1 * q2) / (1 - 2 * (q2 * q2 + q3 * q3)))
            * 180.0
            / math.pi
        )
        return -az, ax
